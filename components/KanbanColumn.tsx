...
export default function KanbanColumn({
  ...
  return (
    <div
      ref={setNodeRef}
      className={`flex flex-col min-w-[240px] w-[240px] md:min-w-[280px] md:w-[280px] transition-all ${
        isOver ? 'bg-surface-secondary/30 rounded-lg p-2' : ''
      }`}
    >
      ...
      <SortableContext items={tasks.map((t) => t.id)} strategy={verticalListSortingStrategy}>
        <div className="flex flex-col gap-4 md:gap-5 min-h-[100px]">
          {tasks.length === 0 ? (
            <div className="text-text-muted text-sm text-center py-8 px-4">
              <p className="mb-2">No tasks yet</p>
              <p className="text-xs">Drag tasks here or click + Add Task</p>
            </div>
          ) : (
            tasks.map((task) => (
